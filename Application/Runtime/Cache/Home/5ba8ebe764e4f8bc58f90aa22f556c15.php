                ...
                <div class="col-xs-12">
                    ...
                    <div class="formArea marT10" >
                            <div class="formTextList" >
                                ...
                                <?php if(is_array($group_data)): $i = 0; $__LIST__ = $group_data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$group_data_vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($group_data_vo['id']); ?>"><?php echo ($group_data_vo['title']); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                            </select>
                            </div>
                    ...
                    <div class="aptTable span10 marT10 tableShow" style="display: none">
                        <div class="aptListBtnGroup " >
                           <input type="checkbox"  id="allCheck" style="position: absolute;top: 50px;left:52.5px;z-index: 99">
                            <a href="javascript:;" id="usergroupAdd"><i class="icon-plus-sign bigger-120 green"></i>添加</a>
                            <a href="javascript:;"  class="tbl_delete" id="groupDel"><i class="icon-remove  bigger-120 red "></i>批量删除</a>
                        </div>
...
<script>
    ...
    $(document).ready(function(){
        function operateFormatter(value, row, index) {
            if(value!=1){
                return [
                    '<a href="/APTSecUI/index.php/Home/User/authorize/id/'+value+'" style="margin-right:4px;"  title="权限设置"><i class="icon-key bigger-130 blue icon-flip-horizontal"></i></a>' +
                    '<a class="green editeUsergroup" href="javascript:;" data="'+value+'" title="编辑"><i class="icon-pencil bigger-130"></i></a> ' +
                    '<a class="red delUsergroup" onclick="tbl_delete('+value+')"  href="javascript:;" data="'+value+'" title="删除"><i class="icon-trash bigger-130"></i></a>'
        ...
        $('#reportTable').bootstrapTable({
            method: 'get',
            url: '/APTSecUI/index.php/Home/User/usergroupOk',
            //search:true,
            cache: false,
            //height: 400,
            striped: true,
	...
	$(document).on("click",".editeUsergroup",function(){	
        ...
        $("#editeusergroup_form input[name='id']").val($(this).attr('data'));
    ...
    function tbl_delete(id) {
        ...
        layer.confirm('您确定要删除这条数据吗？', {
            ...
            $.post("/APTSecUI/index.php/Home/User/usergroupdel", {'id':id}, function (data) {
                console.log(data)
...
//                    console.log(id)
    ...
    $(document).on("click",".tbl_delete",function(){
        ...
        layer.confirm('您确定要删除这些数据吗？', {
            ...
            $.post("/APTSecUI/index.php/Home/User/usergroupdel", {'id':ids}, function (data) {
                if(data.status=='success'){
                    layer.msg(data.msg);
                    window.location.reload();
                    $(":checked").attr('checked',false);
    ...
    $('#editeusergroup_form').submit(function(e){
        ...
        $.ajax({
            url:'/APTSecUI/index.php/Home/User/do_usergroup_edite',
            type:'POST',
            dataType:'json',
            data:data,
    ...
    $('#usergroup_form').submit(function(e){
        ...
        $.ajax({
            url:'/APTSecUI/index.php/Home/User/usergroup_add',
            type:'POST',
            dataType:'json',
            data:usergroup_data,
            success:function(data){
...
///////////////////////------//////////////////////
  jQuery(function ($) {
    ...
    //    send('/APTSecUI/index.php/Home/User/add_web',webdata);
    // });
