                ...
                <div class="col-xs-win">
                    ...
                    <form style="position:relative;margin-top:20px;" id="addUser_form" class="form-horizontal" role="form">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-9">
                                <input type="text" name="name" style="width: 300px" pattern="^\w{1,20}$"  title="请输入小于20字母数学或下划线" required/>
                            </div>
                        </div>
                        <div class="space-4"></div>
                        <div class="form-group">
                            ...
                            <div class="input-group w300 floatLeft" style="width:300px; text-indent:0px;margin-left: 12px" id="ischeck">
                                ...
                                <input class="form-control date-range-picker" type="text" name="date" data-date-format="YYYY-MM-DD" id="id-date-range-picker-1" required />
                ...
                <div class="formArea marT10">
                    <div class="formTextList" style="min-width: 358px">
                        ...
                        <div class="input-group w300 floatLeft" style="width:281px; text-indent:0px;">
                            ...
                            <input class="form-control date-range-picker2" type="text" name="times" data-date-format="YYYY-MM-DD" id="id-date-range-picker-2" />
                        </div>
                    </div>
...
$('#addUser_form').submit(function(e){
    ...
    $.ajax({
        type:"POST",
        url: "/APTSecUI/index.php/Home/Safe/offline_add",
        data: data,
        dataType: "json",
...
jQuery(function ($) {
    $('.date-range-picker').daterangepicker({
                ...
                timePickerIncrement : 360, //时间的增量，单位为分钟
                timePicker:true,
...
function get_offline_num(){
    $.ajax({
        type:'post',
        url:'/APTSecUI/index.php/Home/Safe/get_offline_nums',
        async: false,
        dataType:'json',
...
function get_offline_rate(){
    $.ajax({
        type:'post',
        url:'/APTSecUI/index.php/Home/Safe/get_offline_rate',
        async: false,
        dataType:'json',
        success:function(data){
            get_offline(data);
...
function get_offline(data){
    if(data){
        $.each(data, function(index, value){
            ...
            $('#'+p_rate_w).css('width',value.pickerPercent+'%');
            $('#'+p_rate).html(value.pickerPercent+'%');
            $('#'+a_rate_w).css('width',value.agentPercent+'%');
            $('#'+a_rate).html(value.agentPercent+'%');
            if(value.status=='complete'){
                //console.log(value);
                $.ajax({
                    type:'post',
                    url:'/APTSecUI/index.php/Home/Safe/del_offline_task',
                    async: false,
                    data: {'name':value.Name,'times':value.Time},
                    dataType:'json',
                    success:function(data){
                        if(data.status=='success'){
...
function get_offline_html(num){
    var str = '';
    for(i=1;i<=num;i++){
        str += '<ul class="jinduList" id="jinduList'+i+'"><li style="width: 82px;line-height: 35px" id="num'+i+'"></li>' +
                '<li><li><div class="row" style="width: 168px;margin: 10px 0 0 6px"><div class="beHalf" style="margin-left: 4px"><div class="progress" style="margin-bottom: 12px" ><div class="progress-bar progress-bar-info progress-bar-striped active" style="width: %" id="p_rate_w'+i+'">' +
                '<span class="showing" style="position:relative;top:-6px" id="p_rate'+i+'"></span></div></div></div></div></li><li><div class="row" style="width: 168px;margin: 10px 0 0 6px"><div class="beHalf" style="margin-left: 4px"><div class="progress" style="margin-bottom: 12px"><div class="progress-bar progress-bar-info progress-bar-striped active" style="width: %" id="a_rate_w'+i+'"><span class="showing" style="position:relative;top:-6px" id="a_rate'+i+'"></span></div></div></div></div></li></ul>';
    }
    $('.show_rate').html(str);
}
$(document).ready(function(){
    ...
    $('#reportTable').bootstrapTable({
        method: 'get',
        url: '/APTSecUI/index.php/Home/Safe/offLineOK',
        cache: false,
        //height: 510,
        striped: true,
    ...
    $(document).on('click',".operateStart",function(){
        ...
        $.ajax({
            type:"POST",
            url: "/APTSecUI/index.php/Home/Safe/offline_start",
    ...
    $(document).on('click',".operateStop",function(){
        ...
        $.ajax({
            type:"POST",
            url: "/APTSecUI/index.php/Home/Safe/offline_stop",
            data:{'name':name,'times':times} ,
            async: false,
            dataType:"json",
            success:function(data){
    ...
    $(document).on('click',".operateDelete",function(){
        ...
        $.ajax({
            type:"POST",
            url: "/APTSecUI/index.php/Home/Safe/offline_del",
            data:{'name':name,'times':times} ,
            async: false,
            dataType:"json",
            success:function(data){
